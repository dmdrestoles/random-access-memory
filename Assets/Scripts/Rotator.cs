...
namespace LevelUP.Dial
{
    public class Rotator : MonoBehaviour
    {
        ...
        private void GrabbedBy(SelectEnterEventArgs arg0)
        {
            ...
            interactor.GetComponent<XRRayInteractor>().hideControllerOnSelect = true;
        ...
        private void RotateDialClockwise()
        {
            ...
            float ans = linkedDial.localEulerAngles.z / 25f;
            float copy = ans - (int)ans;
            if (copy >= 0.5f)
            {
                ovenText.text = (((int)(ans + 1)) * 25).ToString();
            }
            else
            {
                ovenText.text = (((int)ans) * 25).ToString();
            }
        }
